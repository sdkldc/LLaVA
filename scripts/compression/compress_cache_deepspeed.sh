...
# bash ./scripts/compression/finetune_token_compress_deepspeed.sh
...
# 포트 충돌 시에 
## --master_port 29501 : 포트 번호 설정
## export MASTER_PORT=29501
## bash ./scripts/compression/compress_cache_deepspeed.sh
# PYTHONNOUSERSITE=1 PYTHONPATH= bash ./scripts/compression/finetune_token_compress_deepspeed.sh


deepspeed --include localhost:1,3--master_port 29501 llava/train/train_mem.py \
    ...
    --bf16 True \
    --output_dir ./checkpoints/llava-v1.5-7b-compress_cache-batch128-token32 \
    --num_train_epochs 1 \
    ...
    --tune_summary_tokens True \
    --use_dual_lora True \
    --kmeans_init False \
    --kmeans_metric cosine \
    --kmeans_n_iter 3 \
    --kmeans_apply_point before_projector \
    --kmeans_use_nearest False \
    --mask_summary_tokens False

# Eager attention 메모리 최적화 설정:
# - batch_size: 16 -> 4 (1/4로 축소)
